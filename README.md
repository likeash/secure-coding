...
- Render의 기본 파일 시스템은 일시적입니다. 무료 Web Service에서 SQLite DB, 세션 DB, 업로드 이미지가 배포·재시작 과정에서 유지된다고 가정하면 안 됩니다.
- 현재 Render 설정은 기능 확인용 데모 배포에 적합합니다. 실제 운영에서는 PostgreSQL 같은 관리형 DB, Redis 기반 세션·rate limit, object storage/CDN으로 이전해야 합니다.
