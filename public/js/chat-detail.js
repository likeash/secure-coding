(function () {
  ...
  var conversationId = Number(box.dataset.conversationId);
  ...
  socket.on('connect', function () {
    socket.emit('chat:join', { conversationId: conversationId }, function (res) {
      if (res && res.ok) {
        joined = true;
        setStatus('');
      } else {
  ...
  function trySend() {
    ...
    socket.emit('chat:message', { conversationId: conversationId, body: body }, function (res) {
      sending = false;
